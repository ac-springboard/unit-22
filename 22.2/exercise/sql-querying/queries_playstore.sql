...
select * from analytics where rating > 4.8 order by reviews desc limit 1; -- or
select * from analytics
where reviews = (select max(reviews) from analytics where rating > 4.8 );

-- 6 --Find the average rating for each category ordered by the highest rated to lowest rated.
select category, round(avg( rating :: numeric), 2) as avgr from analytics group by category order by avgr desc;

-- 7 --Find the name, price, and rating of the most expensive app with a rating that’s less than 3.
select app_name, price, rating from analytics where rating < 3 order by price desc limit 1; -- or
select app_name, price, rating  from analytics
where price = (select max(price) from analytics where rating < 3 );

-- 8 --Find all apps with a min install not exceeding 50, that have a rating. Order your results by highest rated first.
select * from analytics where rating is not null and min_installs <= 50 order by rating desc;
...
select * from analytics order by last_updated limit 1;  -- or
select * from analytics where last_updated = (select min(last_updated) from analytics);

-- 12 --Find the most expensive app (the query is very similar to #11).
select * from analytics order by price limit 1;  -- or
...
select app_name, reviews, min_installs, max(min_installs/reviews) as p
from analytics group by app_name, reviews, min_installs order by p desc limit 1; -- or
select app_name, reviews, min_installs, min_installs/reviews as p
from analytics where min_installs/reviews = (select max(min_installs/reviews) from analytics);
